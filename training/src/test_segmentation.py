...
def run_with_frozen_pb(img_path, input_w_h, frozen_graph, output_node_names):
    ...
    with tf.Session() as sess:
        ...
        mask = heatmaps[0, :, :, :] > threshold

        binary_image = 255.0 * mask
        binary_image = cv2.resize(binary_image, (image_.shape[0], image_.shape[1]))
        show_image = np.copy(image_)
        #print(heatmaps[0, :, :, :] > 0.05)
        show_image[:, :, 0] = show_image[:, :, 0] * (binary_image > 0)
        show_image[:, :, 1] = show_image[:, :, 1] * (binary_image > 0)
        show_image[:, :, 2] = show_image[:, :, 2] * (binary_image > 0)
        print("binary_mask shape = ", binary_image.shape)
        cv2.imshow('image', image_)
        cv2.imshow('result', binary_image)
        cv2.imshow('heat', show_image)
        cv2.waitKey(10000)
